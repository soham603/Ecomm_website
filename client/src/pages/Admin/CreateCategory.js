  ...
  const handledelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `/api/v1/category/delete-categories/${pId}`
      );
      if (data.success) {
        ...
        <div className="row">
          ...
          <div className="col-md-9 ad-bd-title">
            ...
            <div className="p-3 w-50">
              <CategoryForm className="button-full-width button-group"
              ...
              <table className="table">
                ...
                <tbody>
                  {categories?.map((c) => (
                    <>
                      <tr>
                        <td className="ad-bd-keys" key={c._id}>{c.name}</td>
                        <td>
                          <div className="button-group">
                            <button
                              className="btn btn-success button-full-width"
                              onClick={() => {
                                setVisible(true);
                            ...
                            <button
                              className="btn btn-danger button-full-width"
                              onClick={() => {
                                handledelete(c._id);
                              }}
                            >
